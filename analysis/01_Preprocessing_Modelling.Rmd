...
# linear model with mixed effects and time as factor 
...
We adapted a boxed design by individual over Time with a maximum pol<nomial decomposion (n-dim - 1)

```{r modeldef_poly}
models=list(
AV_stressed = "DV~1+age_meancentered+explstart_meancentered_min+BMI_imp_meancentered+any_med_ccept+smoking_yes_no+gender+group+poly(Time, 7)+gender*group+poly(Time, 7)*group+gender*poly(Time, 7)*group+(1|twuid)+(1|centre)",

AV_CORT = "DV~1+age_meancentered+explstart_meancentered_min+BMI_imp_meancentered+any_med_ccept+smoking_yes_no+gender+group+poly(Time, 4)+gender*group+poly(Time, 4)*group+gender*poly(Time, 4)*group+(1|twuid) + (1|centre)",

AV_TEST = "DV~1+age_meancentered+explstart_meancentered_min+BMI_imp_meancentered+any_med_ccept+smoking_yes_no+gender+group+poly(Time, 2)+gender*group+poly(Time, 2)*group+gender*poly(Time, 2)*group+(1|twuid) + (1|centre)",

AV_OXT = "DV~1+age_meancentered+explstart_meancentered_min+BMI_imp_meancentered+any_med_ccept+smoking_yes_no+gender+group+poly(Time, 2)+gender*group+poly(Time, 2)*group+gender*poly(Time, 2)*group+(1|twuid) + (1|centre)")
```
...
# linear model with mixed effects and time as quantiative variable  

We adapted a boxed design by individual over Time with number of minutes as variable of interest

```{r modeldef_quant}
models=list(
AV_stressed = "DV~1+age_meancentered+explstart_meancentered_min+BMI_imp_meancentered+any_med_ccept+smoking_yes_no+gender+group+Time+gender*group+Time*group+gender*Time*group+(1|twuid)+(1|centre)",

AV_CORT = "DV~1+age_meancentered+explstart_meancentered_min+BMI_imp_meancentered+any_med_ccept+smoking_yes_no+gender+group+Time+gender*group+Time*group+gender*Time*group+(1|twuid) + (1|centre)",

AV_TEST = "DV~1+age_meancentered+explstart_meancentered_min+BMI_imp_meancentered+any_med_ccept+smoking_yes_no+gender+group+Time+gender*group+Time*group+gender*Time*group+(1|twuid) + (1|centre)",

AV_OXT = "DV~1+age_meancentered+explstart_meancentered_min+BMI_imp_meancentered+any_med_ccept+smoking_yes_no+gender+group+Time+gender*group+Time*group+gender*Time*group+(1|twuid) + (1|centre)")
```


```{r mml_3, warning=FALSE}
resall = list()
