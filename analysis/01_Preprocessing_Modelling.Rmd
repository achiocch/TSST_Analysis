...
general.model = "DV~1+age_meancentered+explstart_meancentered_min+BMI_imp_meancentered+any_med_ccept+smoking_yes_no+gender+group+poly(Time, 2)+gender*group+poly(Time, 2)*group+gender*poly(Time, 2)*group+(1|twuid)+(1|centre)"

# 
# factor.model = "DV~1+age_meancentered+explstart_meancentered_min+BMI_imp_meancentered+any_med_ccept+smoking_yes_no+gender+group+as.factor(Time)+gender*group+as.factor(Time)*group+gender*as.factor(Time)*group+(1|twuid)+(1|centre)"

models=list(
AV_stressed = general.model,
AV_CORT = general.model,
AV_TEST = general.model,
AV_TESTCORT = general.model,
AV_OXT = general.model)
