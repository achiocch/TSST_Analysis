...
models=list(
AV_stressed = "DV~1+centre+age_meancentered+explstart_meancentered_min+BMI_imp_meancentered+any_med_ccept+smoking_yes_no+gender+group+poly(Time,7)+gender*group+poly(Time,7)*group+gender*poly(Time,7)*group+(1|twuid)",

AV_CORT = "DV~1+centre+age_meancentered+explstart_meancentered_min+BMI_imp_meancentered+any_med_ccept+smoking_yes_no+gender+group+poly(Time,4)+gender*group+poly(Time,4)*group+gender*poly(Time,4)*group+(1|twuid)",

AV_TEST = "DV~1+centre+age_meancentered+explstart_meancentered_min+BMI_imp_meancentered+any_med_ccept+smoking_yes_no+gender+group+poly(Time,2)+gender*group+poly(Time,2)*group+gender*poly(Time,2)*group+(1|twuid)",

AV_OXT = "DV~1+centre+age_meancentered+explstart_meancentered_min+BMI_imp_meancentered+any_med_ccept+smoking_yes_no+gender+group+poly(Time,2)+gender*group+poly(Time,2)*group+gender*poly(Time,2)*group+(1|twuid)")
...
for (depvar in names(AV)){
  ...
  long  = long%>% mutate_if(is.numeric, scale)
  model.lme = lme4::lmer(models[[depvar]], data=long)
