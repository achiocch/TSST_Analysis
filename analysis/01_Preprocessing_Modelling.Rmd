...
df$group = drop(factor(df$group, levels = c(1,2), labels=c("CD", "HCs"))) %>% relevel(., ref="HCs")
...
tmpframe = df[,vartoplot[!vartoplot%in% c("centre")]] %>% mutate_if(is.factor, function(x) as.numeric(x)-1)
...
newnames = c("-", rep(c("beta", "se", "P"), length(names(AV))))
...
#fullres[index,1:16] %>% as.tibble() %>% tableplot_2()
fullres[index,1:16] %>% as.tibble() %>% tableplot_3()
